...
mod_depth_ui <- function(id) {
  ...
  tagList(
    ...
    htmltools::HTML(
      "First, hit the 'Review Depth Data' button to check if there is sufficient depth profile data
    ...
    shiny::conditionalPanel(
      ...
      shiny::HTML(paste0(
        "A total of <strong>",
        shiny::textOutput(ns("number_of_records"), container = tags$span),
...
mod_depth_server <- function(id, tadat) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    shiny::observeEvent(input$review_depth_profile_data, {
      ...
      shinybusy::show_modal_spinner(
        ...
        text = tagList(
          tags$div(
            tags$p(
              "Checking for Depth Profile data",
              tags$br(),
              input$example_data
            ),
    ...
    available_chars_callback <- DT::JS(paste0(
      ...
      "      if(window.Shiny){",
      "        Shiny.setInputValue('",
      ns("available_characteristics_rows_selected"),
      "', sel_idx, {priority: 'event'});",
      "        Shiny.setInputValue('",
      ns("available_chars_overlimit"),
      ...
      input$activity_date,
      {
        ...
        if (length(sel_rows) < 3) {
          ...
          if (length(more_idx) > 0) {
            sel_rows <- c(sel_rows, utils::head(more_idx, 3 - length(sel_rows)))
          }
        }
        ...
        if (length(sel_rows) > 0) {
          ...
          try(
            {
              DT::selectRows(available_chars_proxy, sel_rows)
            },
            silent = TRUE
          )
        }
    ...
    depth_plot_obj <- shiny::eventReactive(input$update, {
      if (
        !depth_profile$loaded || is.null(depth_profile$depth_categorized_df)
      ) {
        shiny::showModal(shiny::modalDialog(
    ...
    depth_plot_data <- shiny::reactive({
      ...
      tidy_long <- df_plot_prep %>%
        ...
        dplyr::summarise(
          value = mean(.value_num, na.rm = TRUE),
          .groups = "drop"
        )
    ...
    output$number_of_records <- shiny::renderText({
      if (!depth_profile$loaded) {
        return("Data not loaded.")
      }
      if (is.null(depth_profile$depth_categorized_df)) {
    ...
    output$number_of_sites <- shiny::renderText({
      ...
      num_sites <- length(unique(as.character(
        depth_profile$depth_categorized_df$TADA.MonitoringLocationIdentifier
      )))
